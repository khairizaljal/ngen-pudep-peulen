...
function bacastatus(){
    xmlhttpPost("GET","/cgi-bin/ajax","status");  
}
...
function pencet(param)
{
  if (document.getElementById('sw'+param).checked == true){
      xmlhttpPost("POST","/cgi-bin/ajax","saklar=on&lampu="+param);
      document.getElementById('tmr'+param).checked=false;}

  else{
      xmlhttpPost("POST","/cgi-bin/ajax","saklar=off&lampu="+param);  
      document.getElementById('tmr'+param).checked=false;}
} 
function pencet2(param)
{
  if (document.getElementById('tmr'+param).checked == true){
      xmlhttpPost("POST","/cgi-bin/ajax","timer=on&lampu="+param);}
  else{
      xmlhttpPost("POST","/cgi-bin/ajax","timer=off&lampu="+param);}  
}
...
function editTimer(param) {
    ...
    if (IN != null) {
         ...
         xmlhttpPost("POST","/cgi-bin/ajax","time=set&value="+IN+"&lampu="+param);
    }
}
...
function pagar(param){
   if(enablepagar=="true"){
    disablebtn();
    enablepagar="false";  
    xmlhttpPost("POST","/cgi-bin/ajax",param);}
}
function disablebtn(){
